...
class TimestepTokenEmbedding(nn.Module):
    def __init__(self, hidden_size: int):
        ...
        self.cos_freq = nn.Linear(1, hidden_size // 2, dtype=torch.bfloat16, bias=False)
        self.sin_freq = nn.Linear(1, hidden_size // 2, dtype=torch.bfloat16, bias=False)

        self.linear_in = nn.Linear(hidden_size, hidden_size, dtype=torch.bfloat16, bias=False)
        self.norm = RMSNorm(hidden_size)
        self.linear_out = nn.Linear(hidden_size, hidden_size, dtype=torch.bfloat16, bias=False)

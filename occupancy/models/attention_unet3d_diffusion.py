...
class TimestepTokenEmbedding(nn.Module):
    def __init__(self, hidden_size: int):
        ...
        self.cos_freq = nn.Linear(1, hidden_size // 2, dtype=torch.bfloat16)
        self.sin_freq = nn.Linear(1, hidden_size // 2, dtype=torch.bfloat16)

        self.linear_in = nn.Linear(hidden_size, hidden_size, dtype=torch.bfloat16)
        self.norm = RMSNorm(hidden_size)
        self.linear_out = nn.Linear(hidden_size, hidden_size, dtype=torch.bfloat16)
...
class TemporalSpatialRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, timestep: Tensor) -> Tensor:
        ...
        hidden_states = hidden_states.view(hidden_states.shape[0], hidden_states.shape[1], -1) + t_bias.view(hidden_states.shape[0], -1, 1)
        hidden_states = hidden_states * torch.rsqrt((hidden_states**2).mean(dim=-1, keepdim=True) + self.eps) * self.scale.view(-1, 1)
...
class TemporalRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, timestep: Tensor) -> Tensor:
        ...
        hidden_states = hidden_states * torch.rsqrt((hidden_states**2).mean(dim=-1, keepdim=True) + self.eps) * self.scale
...
class UnetConvolution3d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.shorcut = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.nonlinear = nn.SiLU(True)
...
class UnetAttention3d(nn.Module):
    def __init__(self, hidden_size: int, condition_size: int, num_heads: int, head_size: int):
        ...
        self.c_proj = nn.Linear(condition_size, hidden_size)
        self.c_norm = TemporalRMSNorm(hidden_size)
    ...
    def forward(
        ...
        cond_embeds = self.c_norm(cond_embeds, timestep)
...
class UnetEncoderLayer3d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, condition_size: int, num_heads: int, head_size: int):
        ...
        self.attention = UnetAttention3d(out_channels, condition_size, num_heads, head_size)
...
class UnetEncoder3d(nn.Module):
    def __init__(
        ...
        self.out_attention = UnetAttention3d(_out_channels[-1], condition_size, _out_channels[-1] // head_size, head_size)

        self.out_norm = TemporalSpatialRMSNorm(_out_channels[-1])
        self.out_conv = nn.Conv3d(_out_channels[-1], latent_dim, kernel_size=1)
        self.nonlinear = nn.SiLU(True)
    ...
    def forward(self, input_embeds: Tensor, cond_embeds: Tensor, timestep: Tensor) -> Tuple[Tensor, List[Tensor]]:
        ...
        latent = self.out_norm(latent, timestep)
...
class UnetDecoderLayer3d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, condition_size: int, num_heads: int, head_size: int):
        super().__init__()
        self.attention = UnetAttention3d(in_channels * 2, condition_size, num_heads, head_size)
        self.upsample = nn.ConvTranspose3d(in_channels * 2, out_channels, kernel_size=2, stride=2)
...
class UnetDecoder3d(nn.Module):
    def __init__(
        ...
        self.in_conv = nn.Conv3d(latent_dim, _in_channels[0], kernel_size=1)
        self.in_attention = UnetAttention3d(
            _in_channels[0],
            condition_size,
        ...
        for i in range(num_layers):
            self.layers.append(UnetDecoderLayer3d(_in_channels[i], _out_channels[i], condition_size, _in_channels[i] // head_size, head_size))
        self.out_norm = TemporalSpatialRMSNorm(_out_channels[-1])
        self.out_conv = nn.Conv3d(_out_channels[-1], out_channels, kernel_size=1)
        self.nonlinear = nn.SiLU(True)
    ...
    def forward(self, latents: Tensor, prev_embeds: List[Tensor], condition_embeds: Tensor, timestep: Tensor) -> Tensor:
        ...
        logits = self.out_norm(hidden_states, timestep)
        logits = self.nonlinear(logits)
        logits = self.out_conv(logits)
        return logits


class UnetMiddleLayer3d(nn.Module):
    def __init__(self, n_channels: int, condition_size: int, num_heads: int, head_size: int):
        super().__init__()
        self.attention = UnetAttention3d(n_channels, condition_size, num_heads, head_size)
...
class UnetBottleNeck3d(nn.Module):
    def __init__(self, n_channels: int, condition_size: int, num_layers: int, head_size: int):
        super().__init__()
        self.layers = nn.ModuleList()
        for i in range(num_layers):
            self.layers.append(UnetMiddleLayer3d(n_channels, condition_size, n_channels // head_size, head_size))
...
class DiffusionAttentionUnet3d(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        self.latent_dim = latent_dim
        self.encoder = UnetEncoder3d(in_channels, latent_dim, condition_size, base_channels, multiplier=multiplier, num_layers=num_layers, head_size=head_size)
        self.middle = UnetBottleNeck3d(latent_dim, condition_size, num_middle_layers, head_size=head_size)
        self.decoder = UnetDecoder3d(out_channels, latent_dim, condition_size, base_channels, multiplier=multiplier, num_layers=num_layers, head_size=head_size)
...
def attention_unet3d(
    ...
    num_middle_layers: int = 3,
    head_size: int = 64,
) -> DiffusionAttentionUnet3d:
    return DiffusionAttentionUnet3d(
        in_channels,
        out_channels,
        latent_dim,
        condition_size,
